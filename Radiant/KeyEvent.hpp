...
namespace Radiant
{
  ...
  public:
    ...
    KeyEvent(const QKeyEvent & event, Source source);
    ...
    /// @param source of keyboard event
    ...
    KeyEvent(int key, QEvent::Type type = QEvent::KeyPress,
             ...
             Source source = VirtualKeyboard, bool autoRepeat = false);
    ...
    /// @return key event
    static KeyEvent createKeyPress(int key, Source source, bool isautorepeat = false);

    /// Creates a key release KeyEvent object with the specified key code
    /// @param key key code of the event
    /// @return key event
    static KeyEvent createKeyRelease(int key, Source source);
