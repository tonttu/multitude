      ...
      */
      ...
      if (m_pkt->stream_index == m_aindex && (m_flags & Radiant::WITH_AUDIO)
        ...
        if(m_audioSampleRate == 44100 || !m_resample_ctx) {
          ...
          debug("resampled: %d; inrate: %d; outrate: %d; srcSz: %d; dstSz: %d", resampled, m_audioSampleRate, 44100, srcSz, dstSz);
        ...
        debug("Decoding audio # %d %lf", aframesOut, m_audioTS.secondsD());
