...
static gboolean g_is_fullscreen = FALSE;
...
{
  if (strncmp("javascript:", url, 11) == 0)
    return NULL;
...
on_stream_open_cb (gpointer user_data)
{
  ...
  if (!pstream || !pstream->np_stream || !pstream->np_stream->url ||
      strncmp("javascript:", pstream->np_stream->url, 11) == 0)
    return FALSE;
